...
const CoreDropdown = ({ options, value, onChange }) => {
  ...
  const baseButton =
    "flex items-center justify-between gap-2 rounded-full px-4 py-2 text-sm font-medium focus:outline-none transition";
  const lightButton =
  ...
  const baseList =
    "absolute left-0 mt-2 w-40 rounded-xl shadow-lg z-50 overflow-hidden";
  const lightList = "bg-white border-gray-200 text-gray-700";
  const darkList = "bg-gray-900 border-gray-600 text-white";

  const baseItem =
    "cursor-pointer px-4 py-2 text-sm transition hover:opacity-80";
  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        className={`${baseButton} ${dark ? darkButton : lightButton}`}
      >
